  ...
  const onSubmit = async (data: FormValues, event: any) => {
    ...
    try {
      const response = await fetch(
        `${REACT_APP_API_ENDPOINT}/auth/signup`,
        requestOptions
      );
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value, ref } }) => (
                  <>
                    ...
                    />
                    {errors.fullName && (
                      <p className={cls.error}>{t("error5")}</p>
                    )}
                  </>
                )}></Controller>
            </div>
                ...
                render={({ field: { onChange, onBlur, value, ref } }) => (
                  <>
                    ...
                    {errors.usernameForSystem && (
                      <p className={cls.error}>{t("error6")}</p>
                    )}
                  </>
                )}></Controller>
            </div>
                ...
                render={({ field: { onChange, onBlur, value, ref } }) => (
                  <>
                    ...
                    {errors?.password?.type === "minLength" && (
                      <p className={cls.error}>{t("error7")}</p>
                    )}
                    {errors?.password?.type === "pattern" && (
                      <p className={cls.error}>{t("error8")}</p>
                    )}
                  </>
                )}></Controller>
            </div>
